  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      forkJoin({
        ...
        console.log(info);
        this.measurementService.measurementNotificationActivity.subscribe(
          (resultData) => {
            const testStatus = resultData.testStatus;
            const passedResults = resultData.passedResults;
        ...
        this.measurementService
          .start(
            measurementRecord.mlabInformation.fqdn,
            3001,
            '/ndt_protocol',
            200
          )
          .then(
            async (passedResults) => {
              ...
              const enabled = this.settingsService.get('uploadEnabled');
              if (enabled) {
                this.uploadService
